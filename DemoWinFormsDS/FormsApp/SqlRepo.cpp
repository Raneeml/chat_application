...
namespace DBRepository {
	...
	{
		return createTable(keyColumnNames, false);
	}

	generic <class T>
	bool SqlRepo<T>::createTable(String^ keyColumnNames, bool autoIncrementId)
	{
		assert(!tableExists());
	...
	String^ SqlRepo<T>::prepareInsertionData(T instance, Reflection::FieldInfo^ prop, bool isFirstColumn)
	{
		...
		if (isFirstColumn && fieldDataType->StartsWith("Int")) {
			return "";
		}
